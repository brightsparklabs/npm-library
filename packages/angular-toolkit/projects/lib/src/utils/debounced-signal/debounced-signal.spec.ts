...
describe("debouncedSignal", () => {
  ...
  it("Should hold initialValue and debounce updates", async () => {
    ...
    const debounced = runInInjectionContext(injector, () => debouncedSignal(source, "data", 100));
  ...
  it("Should emit immediately when debounce time is 0", async () => {
    const injector = TestBed.inject(EnvironmentInjector);
    const source = signal("initial");
    const debounced = runInInjectionContext(injector, () => debouncedSignal(source, "initial", 0));
